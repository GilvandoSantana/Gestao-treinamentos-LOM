...
async function startServer() {
  ...
  // do mês, pra colocar no cartaz do Canva. Mesmo esquema da rota anterior
  // (segredo fixo, será REMOVIDA assim que o cartaz estiver pronto). Só
  // devolve nome e URL da foto (já pública no Supabase) de quem tem foto
  // cadastrada — nada mais sensível.
  app.get("/api/temp-birthday-photos", async (req, res) => {
    ...
    const results = employees
      .filter((e: any) => !e.dismissed && e.birthDate && e.photoUrl)
      .map((e: any) => {
        const [, month, day] = String(e.birthDate).split("-").map(Number);
        return { name: e.name, day, month, photoUrl: e.photoUrl };
      })
